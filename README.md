...
default — Drippy speaks in color and posture, never sounds or popups.
...
2. First launch: this build isn't notarized yet, so macOS will refuse it once.
...
Requirements: Apple Silicon Mac, macOS 13+. Everything runs locally — no
account, no cloud. See [PRIVACY.md](PRIVACY.md).
...
future community/aggregate view will be strictly opt-in.
...
Impact math lives in [impact.js](impact.js) over
[impact-factors.json](impact-factors.json) — arithmetic and lookup tables,
...
[renderer/](renderer/) — pure CSS per the design handoff in
...
classes (`has-eyes`, `has-gaze`, `has-glow`, `has-lean`) plus whole-body
modes for privacy and footprint.

### Release checklist (not yet done)

- [ ] Apple Developer ID signing + notarization (removes the Open Anyway step)
- [ ] Universal binary (Intel + Apple Silicon)
- [ ] Auto-update feed
- [ ] Choose a license (currently UNLICENSED — all rights reserved)
- [ ] Publish source

## Troubleshooting

- **Blob invisible:** it may be behind a fullscreen space edge — the 💧 menu
  is always there; quit and relaunch to reset position by deleting
